...
public interface BookRepository extends DataTablesRepository<Book, Long> {
	@Query(value = "select book.book_id, book.name, book.author_id, book.category_id, book.image, book.status, book.description, count(order_detail.book_id) as count from order_detail join book on order_detail.book_id= book.book_id where month(order_detail.end_date)  between 5  and 6 group by book.book_id order by  count desc limit 3;", nativeQuery = true)
	List<Book> topBook();
}
